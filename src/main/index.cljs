(ns main.index
  (:require
   [main.sketches.sketch-01 :as sketch]))
      ...
      (let [now (.now js/Date)
        ...
        (when (or (:noloop opts) (>= elapsed fps-interval))
          ...
          (let [{:keys [rows cols]} @state]
            (doseq [r (range rows)
                    c (range cols)]
              (let [context (assoc @state :x c :y r)
...
(defn ^:dev/after-load start
  ...
  (let [opts (merge opts (or sketch/opts {}))]
    (run opts sketch/setup sketch/draw)))
